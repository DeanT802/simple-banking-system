    ...
    def clean_amount(self):
        ...
        if amount < min_deposit_amount:
            raise forms.ValidationError(
                f'You need to deposit at least Rp {min_deposit_amount}'
            )
...
class WithdrawForm(TransactionForm):
    ...
    def clean_amount(self):
        ...
        if amount < min_withdraw_amount:
            raise forms.ValidationError(
                f'You can withdraw at least Rp {min_withdraw_amount}'
            )

        if amount > max_withdraw_amount:
            raise forms.ValidationError(
                f'You can withdraw at most Rp {max_withdraw_amount}'
            )

        if amount > balance:
            raise forms.ValidationError(
                f'You have Rp {balance} in your account. '
